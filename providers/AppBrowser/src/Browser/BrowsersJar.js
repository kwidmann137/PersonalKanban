...
module.exports = function (launchOptions) {
  class BrowsersJar {
    constructor (BaseRequest, BaseResponse, auth, assert) {
    ...
    async loginViaJwt (user, password) {
      const jwt = await this._auth.attempt(user.email, password);
      ...
      this.constructor.defaultBrowser._token = JSON.stringify(jwt);
      this.constructor.defaultBrowser._loggedIn = true;
    };
